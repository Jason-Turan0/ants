...
class MapViewSequence(FileSystemSequence):
    ...
    def get_map_feature_path(self, game_path: str) -> str:
        return game_path.replace('.json',
                                 f'_MAPVIEW_FEATURES_{self.bot_to_emulate}_{self.channel_count}.npy')

    def get_map_label_path(self, game_path: str) -> str:
        return game_path.replace('.json',
                                 f'_MAPVIEW_LABELS_{self.bot_to_emulate}_{self.channel_count}.npy')

    def get_index_path(self, game_path: str) -> str:
        return game_path.replace('.json',
                                 f'_MAPVIEW_index_{self.bot_to_emulate}_{self.channel_count}.txt')
    ...
    def build_indexes(self, rebuild: bool):
        ...
        for game_path in self.game_paths:
            if os.path.exists(self.get_index_path(game_path)) and not rebuild:
                ...
                maps = self.gst.convert_to_global_antmap(self.bot_to_emulate, [gs])
                map_features, map_labels = enc.encode_map_examples(maps, self.channel_count)
                shuffled_map_features, shuffled_map_labels = sk.utils.shuffle(map_features, map_labels)

                print(f'Saving {map_features.shape[0]} examples to {self.get_map_feature_path(game_path)}')
                ...
                self.write_config(self.get_index_path(game_path), map_features.shape[0])
                self.game_indexes.append(self.read_config(game_path, self.get_index_path(game_path)))
    ...
    def load_index(self, index: GameIndex) -> LoadedIndex:
        ...
        if loaded_index is not None:
            ...
            self.loaded_indexes.append(LoadedIndex(index, map_features, map_labels))
    ...
    def get_feature_shape(self, index_range: Tuple[int, int]) -> List[tuple]:
        ...
        return [(range_len, 49, 39, self.channel_count)]
