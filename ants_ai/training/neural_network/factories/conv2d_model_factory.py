...
class Conv2DModelFactory(ModelFactory):
    ...
    def construct_model(self, tuned_params: Dict[str, Union[int, float]], hps: HyperParameters = None) -> Model:
        ...
        model = Sequential([
            Input(name='Conv2D_Input', shape=(12, 12, 7)),
            Conv2D(filter_0, 2, strides=1, activation=tf.nn.relu, name='Conv2D_Conv2D_1'),
            Conv2D(filter_1, 3, strides=1, activation=tf.nn.relu, name='Conv2D_Conv2D_2'),
            Conv2D(filter_2, 2, strides=1, activation=tf.nn.relu, name='Conv2D_Conv2D_3'),
            Flatten(name='Conv2D_Flatten'),
            Dropout(0.1, name='Conv2D_Dropout'),
            Dense(dense, activation=tf.nn.relu, name='Conv2D_Dense'),
            Dense(5, activation=tf.nn.softmax, name='Conv2D_Output'),
        ])
