...
class HybridModelFactory(ModelFactory):
    ...
    def construct_model(self, tuned_params: Dict[str, Union[int, float]]) -> Model:
        ...
        avm = Conv2D(self.get_param_value(FILTER0_NAME, tuned_params), 2, strides=1, activation=tf.nn.relu,
                     name='Conv2D_av1_32')(input_ant_view)
        avm = Conv2D(self.get_param_value(FILTER1_NAME, tuned_params), 3, strides=1, activation=tf.nn.relu,
                     name='Conv2D_av2_64')(avm)
        avm = Conv2D(self.get_param_value(FILTER2_NAME, tuned_params), 2, strides=1, activation=tf.nn.relu,
                     name='Conv2D_av3_128')(avm)
        avm = Flatten(name='Flatten_av')(avm)
        avm = Dense(self.get_param_value(DENSE_NAME, tuned_params), activation=tf.nn.relu, name='Dense_av')(avm)
        ...
        mvm = Conv2D(32, 2, strides=1, activation=tf.nn.relu, name='Conv2D_mv1_32')(mvm)
        mvm = Conv2D(64, 3, strides=1, activation=tf.nn.relu, name='Conv2D_mv2_64')(mvm)
        mvm = Conv2D(128, 2, strides=1, activation=tf.nn.relu, name='Conv2D_mv3_128')(mvm)
    ...
    def construct_discover_model(self, hps: HyperParameters) -> Model:
        filter_0 = self.get_hyper_param(FILTER0_NAME,
                                        lambda default: hps.Choice(FILTER0_NAME, values=[4, 8, 16, 32],
                                                                   default=default))
        filter_1 = self.get_hyper_param(FILTER1_NAME, lambda default: hps.Choice(FILTER1_NAME, values=[32, 48, 64],
                                                                                 default=default))
        filter_2 = self.get_hyper_param(FILTER2_NAME,
                                        lambda default: hps.Choice(FILTER2_NAME, values=[64, 96, 128], default=default))
        dense = self.get_hyper_param(DENSE_NAME, lambda default: hps.Int(DENSE_NAME, 32, 128, step=8, default=default))
        ...
        input_ant_view = Input(shape=(12, 12, 7), name='input_ant_view')
        avm = Conv2D(filter_0, 2, strides=1, activation=tf.nn.relu, name='Conv2D_av1')(input_ant_view)
