...
def map_to_input(pr: Tuple[str, PlayResult]) -> List[NeuralNetworkExample]:
    print('Parsing game ' + pr[1].game_id)
    generator = GameStateGenerator()
    translator = GameStateTranslater()
    input = translator.convert_to_nn_input(pr[0], [generator.generate(pr[1])])
    #print('Done parsing ' + pr[1].game_id)
    return input.examples

def create_test_examples(game_count: int, bot_name: str) -> List[NeuralNetworkExample]:
    files = [f for f in glob.glob(f'{os.getcwd()}\\training\\tests\\test_data\\tournament 2020-08-07-23-59-20\\*.json')]

    play_results = seq([get_play_result(file) for file in files]) \
        .filter(lambda pr: bot_name in pr.playernames) \
        .take(game_count) \
        .map(lambda pr: (bot_name, pr))\
        .to_list()
