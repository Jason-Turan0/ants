...
class CellMazeMap(Map):
    ...
    def generate(self):
        ...
        def pick_size(grid=None, aspect_ratio=None):
            ...
            # the min/max values for aspect ratio are tuned for more grid rows
            # than columns
            if row_sym > col_sym:
        ...
        def make_symmetric(points, size, players, grid, v_sym=None, v_step=None, h_sym=None, h_step=None):
            ...
            def extend(funcs, points, size, count=2, step=0, shear=1):
                # shear is used for a MxN grid where shearing occurs in both
                # directions
                ...
                for sym_points in points:
                    ...
                    for point in sym_points:
                        for c in range(1,count):
                            new_sym_points.append(point_func(point,                    # point
                ...
                for col in range(cols):
                    ...
                    if len(closest) > 1:
                        if len(comps) > 1:
                            ...
                            # find all starting points that contributed to the
                            # water wall,
                            # add to water wall dict
#                        else:
#                            if distance(closest[0][0], closest[1][0], size) >
#                            self.cell_width:
#                                comps = (comps[0], comps[0])
#                                # store unique points in path structure so we
#                                can later check if traversable
...
#                    if not self.get_path(closest[0][0], closest[1][0], size,
#                    3):
...
#                        break
                    ...
                    if len(path) == 2:
                        if not any(self.get_path(starts_by_comp[path[0]][0], dest_loc, size, 3, paths[path])
                            ...
                            if neighbor_dist(neighbors, path[0], path[1]) is not None:
                                ...
                                # we should restart the path checks since the
                                # openings between cells have changed
                                break
                else:
        ...
        def ensure_connected(paths, carved_paths, carve):
            ...
            # I'm too lazy to search for disconnected nodes and fuse cells for
            # proper 3x3 block traversal
            ...
            if len(sections) > 1:
                ...
                # look for largest blocks of squares first, since this will
                # make it easiest to traverse
