...
public class Strategy {
    ...
    private void initTurn() {
        ...
        for (Tile hill : hills) {
            if (hill.hillPlayer == Type.MY_ANT) {
                myHills.add(hill);
                logger.print(hill.toString());
    ...
    private void doMission(Mission m) {
        ...
        if (dest == null) {
            ...
            logger.println("remove mission because cannot savely reach " + m.target + " from " + m.currTile);
            return;
        }
        ant.hasMission = true;
    ...
    private void defendHill(Tile hill) {
        ...
        enemyLoop:
        for (Tile enemyTile : dangerousEnemyTiles) {
    ...
    private void fight() {
        for (Area area : fightAreas) {
            for (Ant startAnt : area.ants) {
                if (startAnt.hasMoved || startAnt.gammaDistEnemies.size() == 0 || startAnt.numCloseEnemies == 0 || startAnt.isGammaGrouped)
                    continue;
    ...
    private void findPrecGroup(Ant startAnt) {
        ...
        while (!myOpenList.isEmpty()) {
            ...
            if (myMoveSafeCount + myMoveTestCount > MY_MOVE_MAX) doesFit = false;
                ...
                if (myMoveSafeCount + myMoveTestCount > 17 - 3 && enemySafeCount + enemyTestCount > 5 - 1)
                    doesFit = false;
                else if (myMoveSafeCount + myMoveTestCount > 13 - 3 && enemySafeCount + enemyTestCount > 8 - 1)
                    doesFit = false;
            }
            if (doesFit) {
            ...
            if (enemyAnt.willStay) {
                ...
                if (isBetaDist(enemyAnt.tile, dest)) {
                    if (enemyAnt.tile.neighbors.length == 4) {
                        if (dangered) return true;
                        dangered = true;
                    } else for (Tile n : enemyAnt.tile.neighbors)
                        if (isAlphaDist(n, dest)) {
                            if (dangered) return true;
                            dangered = true;
                        }
                }
