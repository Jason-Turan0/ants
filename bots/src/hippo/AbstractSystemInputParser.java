...
public abstract class AbstractSystemInputParser extends
    ...
    private enum SetupToken {
        ...
        private static final Pattern PATTERN =
                Pattern.compile("LOADTIME|TURNTIME|ROWS|COLS|TURNS|VIEWRADIUS2|ATTACKRADIUS2|SPAWNRADIUS2");
    }

    private enum UpdateToken {
        W, A, F, D, H;
        private static final Pattern PATTERN = Pattern.compile("W|A|F|D|H");
    }
    ...
    public void processLine(String line) {
        ...
        if (line.equals(READY)) {
            ...
            LogFile.write(input, "ready");
            if (AbstractSystemInputReader.turnOutput) {
                LogFile.write("c:\\Eclipse\\workspace\\ants\\Turns\\" + getTurnFileName(), input, "ready");
            }
            parseSetup(input);
            int time = doTurn();
            afterTurn();
            // after playgame study I have discovered the bot could be paused immediately after sending "go",
            // but I am lazy to put it afterTurn back to doTurn
            finishTurn(time);
            input.clear();
        } else if (line.equals(GO)) {
            Ants.setTurnStartTime(System.currentTimeMillis());
            LogFile.write(input, "go");
            if (AbstractSystemInputReader.turnOutput) {
                LogFile.write("c:\\Eclipse\\workspace\\ants\\Turns\\" + getTurnFileName(), input, "go");
            }
            parseUpdate(input);
            int time = doTurn();
            afterTurn();
    ...
    public void finishTurn() {
        ...
        //System.out.flush();
        LogFile.writeFlush(">go ");
    }

    public void finishTurn(long time) {
        LogFile.write(">go " + time);
        //System.out.println("go");
        //System.out.flush();
    }
