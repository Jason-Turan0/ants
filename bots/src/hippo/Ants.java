...
public class Ants {
	...
	public Ants(int loadTime, int turnTime, int rows, int cols, int turns,
		...
		mybot.init(this);
		Set<Ilk> mA=new HashSet<Ilk>(); mA.add(Ilk.MY_ANT); 
		myAnts = new TileList(mA,null);
		Set<Ilk> mTA=new HashSet<Ilk>(); mTA.add(Ilk.MY_TMP_ANT); 
		myTmpAnts = new TileList(mTA,null);
		Set<Ilk> mAP=new HashSet<Ilk>(); mAP.add(Ilk.MY_ANT_PLANNED); 
		myAntsPlanned = new TileList(mAP,null);
		Set<Ilk> eA=new HashSet<Ilk>(); eA.add(Ilk.ENEMY_ANT); 
		enemyAnts = new TileList(eA,null);
		Set<HillTypes> mH=new HashSet<HillTypes>(); mH.add(HillTypes.MY_HILL);
		myHills = new TileList(null,mH);
		Set<HillTypes> eMH=new HashSet<HillTypes>(); eMH.add(HillTypes.EXPECTED_MY_HILL);
		myExpectedHills = new TileList(null,eMH);
		Set<HillTypes> eH=new HashSet<HillTypes>(); eH.add(HillTypes.ENEMY_HILL); eH.add(HillTypes.SYMMETRY_ENEMY_HILL);
		enemyHills = new TileList(null,eH);
		Set<HillTypes> eEH=new HashSet<HillTypes>(); eEH.add(HillTypes.EXPECTED_ENEMY_HILL); eEH.add(HillTypes.SYMMETRY_ENEMY_HILL);
		enemyExpectedHills = new TileList(null,eEH);
		Set<HillTypes> dH=new HashSet<HillTypes>(); dH.add(HillTypes.DEAD);
		deadHills = new TileList(null,dH);
		Set<Ilk> fT=new HashSet<Ilk>(); fT.add(Ilk.FOOD);
		foodTiles = new TileList(fT,null);
		Set<Ilk> pFT=new HashSet<Ilk>(); pFT.add(Ilk.PLANNED_FOOD);
		plannedFoodTiles = new TileList(pFT,null);
		Set<Ilk> pSFT=new HashSet<Ilk>(); pSFT.add(Ilk.PLANNED_SYMMETRY_FOOD);
		plannedSymmetryFoodTiles = new TileList(pSFT,null);
		Set<Ilk> pXFT=new HashSet<Ilk>(); pXFT.add(Ilk.PLANNED_EXPLORATION_FOOD);
		plannedExplorationFoodTiles = new TileList(pXFT,null);
		Set<Ilk> eFT=new HashSet<Ilk>(); eFT.add(Ilk.EXPECTED_FOOD);
		expectedFoodTiles = new TileList(eFT,null);
		Set<Ilk> sFT=new HashSet<Ilk>(); sFT.add(Ilk.SYMMETRY_FOOD);
		symmetryFoodTiles = new TileList(sFT,null);
		Set<Ilk> xFT=new HashSet<Ilk>(); xFT.add(Ilk.EXPLORATION_FOOD);
		explorationFoodTiles = new TileList(xFT,null);
		Set<Ilk> dA=new HashSet<Ilk>(); dA.add(Ilk.DEAD);
	...
	public void see(int row,int col) {
		int mrow=(row+rows)%rows,mcol=(col+cols)%cols; Ilk ilk;
	...
	public void attackUpdate() {
		...
		myAttackUpdate();
		for (Tile enemyAttack=getEnemyAttackTiles().getFirstNoFilter();enemyAttack!=null;enemyAttack=getEnemyAttackTiles().getNextNoFilter()) {
			int attack=getEnemyAttack(enemyAttack);Tile[] attackers=getAttackedByEnemy(enemyAttack);
			if ((attack>=1)&&(attack<=2)) {
		...
		for (Tile myAttack=getMyAttackTiles().getFirstNoFilter();myAttack!=null;myAttack=getMyAttackTiles().getNextNoFilter()) {
			int attack=getMyAttack(myAttack);Tile[] attackers=getAttackedByMy(myAttack);
			if ((attack>=1)&&(attack<=2)) {
				boolean attackPlaceFound=false;
				for (Tile attacker:attackers) {
					for (Aim direction : Aim.permAim[0]) {
	...
	public int travelDistance(Tile t1, Tile t2, int infty) {// distance avoiding watter. Going through enemy ants is OK
		int dist=0;search.clear();
		search.addNotVisited(t1);boolean insertBreak=true;
				...
				for (Aim direction : Aim.permAim[permId]) {
					...
					if (search.notVisited(moved)) {
						if (getIlk(moved).equals(Ilk.MY_ANT)) {
							result.add(moved);result.add(targetNeighbour);
	...
	public Aim getDirection(Tile t1,Tile t2) {
		for (Aim direction : Aim.values()) {
			...
			if (test.equals(t2)) {return direction;}
		}
	...
	public void frontLineUpdate(TileList tmpFree,TileList tmpNotFree) {
		...
		for(Tile front=tmpNotFree.getFirstNoFilter();front!=null;front=tmpNotFree.getNextNoFilter()) {
			...
			for (Aim direction: Aim.permAim[0]) {
				Tile moved=getTile(front,direction);Ilk ilk=getIlk(moved);
				if (ilk.isStepable()) {
